...
function Aptiadd() {
  ...
  const removeQuestion = async (questionId) => {
    try {
      await axios.delete(`http://localhost:5000/questionData/${questionId}`);
      fetchQuestions();
    } catch (error) {
      ...
      <div className='container-fluid bg-secondary min-vh-100'>
        <div className='row'>
          ...
          <div className='col'>
            ...
            <Link to='/insert'><button className="btn btn-dark p-2 mt-2" >Add Question</button></Link>
          </div>
        </div>
      </div>
    </div>
