...
function Verbal() {
    ...
    const [timerStopped, setTimerStopped] = useState(true); // Initialize timer stopped
    const [attemptedQuestions, setAttemptedQuestions] = useState(0); // Track number attm questions
    ...
    const fetchQuestions = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5000/verbalques');
    ...
    const handleSubmit = async () => {
        ...
        const resultData = {
            ...
            testType: 'Verbal Test', // Replace 'current_affairs' with the actual test type
    ...
    }, [timeLeft, timerStopped, handleSubmit]);

    // Function to start the test
    const startTest = () => {
        setTimerStopped(false); // Start the timer
    };
    ...
    useEffect(() => {
        const showConfirmation = () => {
            confirmAlert({
                ...
                buttons: [
                    {
                        label: 'Yes',
                        onClick: startTest // Start the test when user clicks Yes
                    },
                    {
                        label: 'No',
                        onClick: () => {
        ...
        showConfirmation(); // Show the confirmation dialog when the component mounts
    ...
    return (
        <div className='px-3'>
            <hr /><h1 style={{ background: "grey" }}>Welcome to Letsdoit!.com</h1><hr />
